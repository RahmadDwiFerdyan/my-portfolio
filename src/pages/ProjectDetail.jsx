...
function ProjectDetailInner({ id, navigate }) {
  ...
  const handleBackToHome = () => {
    ...
    if (lastScrollY != null)
      sessionStorage.setItem("pendingHomeScrollY", lastScrollY);
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 gap-16 md:grid-cols-12">
        {/* LEFT STICKY MENU */}
        <aside className="md:col-span-3 lg:col-span-3 md:sticky md:top-16 self-start">
          <div className="pb-4">
            <button
              type="button"
                ...
                {sections.map((s) => {
                  ...
                  return (
                    <button
                      ...
                      className={
                        isActive
                          ? "flex items-center gap-3 rounded-xl border border-primary/30 bg-primary/5 px-3 py-2 text-left text-primary text-sm font-semibold"
                          : "flex items-center gap-3 rounded-xl border border-white/10 bg-white/0 px-3 py-2 text-left text-white/90 text-sm font-semibold hover:border-white/20 hover:bg-white/5 hover:text-primary transition"
                      }
          ...
          {isLanguageSwitching ? (
            <div className="pointer-events-none absolute inset-0 z-20 flex items-start justify-center pt-8">
              <div className="inline-flex items-center gap-3 rounded-full border border-white/15 bg-[#0B1C2D]/85 px-4 py-2 text-sm font-semibold text-white shadow-lg backdrop-blur-md">
                <span
                  className="h-4 w-4 rounded-full border-2 border-primary/40 border-t-primary animate-spin"
                  aria-hidden="true"
                />
                Updating language...
              </div>
            ...
            {markdown ? (
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
                <p className="text-white/80">
                  No case study content found for this project. Add a Markdown
                  file under
